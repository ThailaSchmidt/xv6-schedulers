...
// isso é p round robin (lembrar em qual process parou)
...
// numero de bilhetes para cada classe
#define CLASS_0_BILHETES 6
#define CLASS_1_BILHETES 3
#define CLASS_2_BILHETES 2
#define CLASS_3_BILHETES 1

enum priority_class {
  CLASS_0,  // maior prioridade
  CLASS_1,
  CLASS_2,
  CLASS_3   
};
...
//trab2 ////////////
#define TOTAL_TICKETS 10000 //valor que sera usado para descobrir o numeor de passos
...
int scheduler_mode = 1; // 0 = loteria, 1 = stride
void scheduler(void) {
  for (;;) { //deve ser infinito
    if (scheduler_mode == 0){
      scheduler_loteria();
    }else{
      scheduler_stride();
...
void scheduler_loteria(void) {
  struct cpu *c = mycpu();
  struct proc *p;
  enum priority_class classe_atual;
  ...
  for (;;) {
    ...
    for (i = 0; i < NPROC; i++) {
      ...
      if (p->state == RUNNABLE && p->classe == classe_atual) { //só roda se RUNNABLE e pertence a classe sorteada
        ...
        //atualiza o indice para executar o proximo processo da proxima vez (round robin)
...
void scheduler_stride(void) {
  struct cpu *c = mycpu();
  struct proc *p;
  struct proc *min_pass_proc = 0; //menor passo
  ...
  for (;;) {
    ...
    //escalona o processo com menor pass
    if (min_pass_proc) {
      //printf("min do if: %d\n", min_pass_proc->pass);
...
found:
  ...
  //  já que o comportamento especial de escolha de prioridade só é feito via forkprio()
...
int forkprio(int prio) {
    ...
    // Copia a memória do processo pai para o filho
    ...
    // Copia os registradores salvos
    *(np->trapframe) = *(p->trapframe);

    // Faz o fork retornar 0 no filho
    np->trapframe->a0 = 0;

    // Incrementa as referências dos descritores de arquivos abertos
    for(i = 0; i < NOFILE; i++) {
        if(p->ofile[i]) {
